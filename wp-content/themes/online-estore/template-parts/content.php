...
$post_meta = get_theme_mod( 'online_estore_meta_posts_option', 'on' );
...
<article id="post-<?php the_ID(); ?>" <?php post_class('article wow fadeInUp'); ?>>
	...
	<?php
        online_estore_post_format_media( $postformat );
	?>
	...
	<div class="box">
		...
		if( is_single( ) ){
			...
			if( !empty( $post_meta ) && $post_meta == 'on' ){
			
			if ( 'post' === get_post_type() ) : ?>

				<div class="site-entry-meta metainfo">
					<?php
						online_estore_posted_on();
						online_estore_posted_by();
						online_estore_comments();
					?>
				</div><!-- .entry-meta -->
				
		<?php endif;  } }else{ if ( 'post' === get_post_type() ){ do_action( 'online_estore_post_meta', 10 ); } } ?>
